...
# 3 smoothly transitioning power laws
def log_smooth_powerlaw(log_v, logA, a1, a2, a3, vb, vc, k, m):
    v = 10**log_v
    base = logA + a1 * log_v
...
# Velocity calculation from collection efficiency
def velocity_from_qi_qt(qi, qt, c=55.0, p=-3.2, v0=1.5):
    """
    Compute impact speed from the ratio QI/QT using the empirical relation:
        v(QI/QT) = c * (QI/QT)^p + v0

    Parameters:
    - qi : float or array-like
        Ion grid charge (QI)
    - qt : float or array-like
        Target charge (QT)
    - c : float
        Scaling constant (default = 55.0 km/s)
    - p : float
        Power-law exponent (default = -3.2)
    - v0 : float
        Offset velocity (default = 1.5 km/s)

    Returns:
    - v : float or array-like
        Estimated velocity [km/s]
    """
    ratio = qi / qt
    return c * ratio**p + v0
...
def FitTargetSignal(time, targetAmp):
    ...
    mask = np.logical_and(time >= -60, time <= -50)
    print(mask)
