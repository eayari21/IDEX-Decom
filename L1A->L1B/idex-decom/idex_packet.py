...
class IDEXEvent:
        ...
        for pkt in idex_packet_generator:
            ...
            if 'IDX__SCI0TYPE' in pkt.data:
                ...
                if pkt.data['IDX__SCI0TYPE'].raw_value == 1:
                    ...
                    self.hspretrigblocks = (pkt.data['IDX__TXHDRBLOCKS'].derived_value >> 16) &  0b1111

                    # Extract the next 4-bit integer (usually 8)
                    self.hsposttrigblocks = (pkt.data['IDX__TXHDRBLOCKS'].derived_value >> 12) & 0b1111

                    # Extract the next 6 bits integer (usually 32)
                    self.lspretrigblocks = (pkt.data['IDX__TXHDRBLOCKS'].derived_value >> 6) & 0b111111

                    # Extract the first 6 bits (usually 32)
                    self.lsposttrigblocks = (pkt.data['IDX__TXHDRBLOCKS'].derived_value) & 0b111111
                    ...
                    if(pkt.data['IDX__TXHDRLGTRIGMODE'].derived_value!=0):
                        ...
                        self.header[(evtnum, 'TriggerLevel')] = (pkt.data['IDX__TXHDRLGTRIGCTRL1'].derived_value >> 22) & mask_10_bit
                    ...
                    if(pkt.data['IDX__TXHDRMGTRIGMODE'].derived_value!=0):
                        ...
                        # Extract the last 10 bits (bits 0-9)
                        self.header[(evtnum, 'TriggerLevel')] = (pkt.data['IDX__TXHDRMGTRIGCTRL1'].derived_value >> 22) & mask_10_bit
                        print(f"Trigger level = {self.header[(evtnum, 'TriggerLevel')]}")
    ...
    def plot_all_data(self, packets, fname: str):
        ...
        for i, (k, v) in enumerate(packets.items()):  # k[0] = Event number, k[1] = CHnel name, v=waveform data
            ...
            if(i<=2):
                self.hstriggertime = 512*(1/260)*(self.hspretrigblocks) - (1/260)*(self.hgdelay+1)

                print(f"High sampling trigger time = {self.hstriggertime} microseconds.")
                self.hstime = self.hstime #- self.hstriggertime
                ax[i].axvline(min(self.hstime)+self.hstriggertime, c="red", lw=2)
                # ax[i].axhline(self.header[(1, 'TriggerLevel')], c="red", lw=2)

            else:
                self.lstriggertime = 8*(1/4.0625)*(self.lspretrigblocks) # - (1/260)*self.hgdelay
                print(f"Low sampling trigger time = {self.lstriggertime} microseconds.")
                self.lstime = self.lstime-self.lstriggertime
                ax[i].axvline(min(self.lstime)+self.lstriggertime, c="red", lw=2)
            ...
            if i==5:  #  End of the event, lets free up some memory
                # print(len(self.hstime), len(self.lstime))

                ax[0].plot(self.hstime, conversion_factors["TOF L"] * np.array(packets[(k[0], "TOF L")]))
                ax[0].set_ylabel("TOF L", font="Times New Roman", fontsize=15, fontweight='bold')
                text = f'Min = {min(conversion_factors["TOF L"] * np.array(packets[(k[0], "TOF L")])): 4.2f} [pC]'+ '\n'+ f'Avg={np.mean(conversion_factors["TOF L"] * np.array(packets[(k[0], "TOF L")])): 4.2f} [pC]'+ '\n' + f'Std={np.std(conversion_factors["TOF L"] * np.array(packets[(k[0], "TOF L")])): 4.2f} [pC]'+ '\n' + f'Max = {max(conversion_factors["TOF L"] * np.array(packets[(k[0], "TOF L")])): 4.2f} [pC]'
                ax[0].text(1.125, 0.85, text, fontsize=12, va="top", ha="right", transform=ax[0].transAxes)
                # ax[0].set_xlim([0, 31.5])
                
                ax[1].plot(self.hstime, conversion_factors["TOF M"] * np.array(packets[(k[0], "TOF M")]))
                ax[1].set_ylabel("TOF M", font="Times New Roman", fontsize=15, fontweight='bold')
                text = f'Min = {min(conversion_factors["TOF M"] * np.array(packets[(k[0], "TOF M")])): 4.2f} [pC]'+ '\n'+ f'Avg={np.mean(conversion_factors["TOF M"] * np.array(packets[(k[0], "TOF M")])): 4.2f} [pC]'+ '\n' + f'Std={np.std(conversion_factors["TOF M"] * np.array(packets[(k[0], "TOF M")])): 4.2f} [pC]'+ '\n' + f'Max = {max(conversion_factors["TOF M"] * np.array(packets[(k[0], "TOF M")])): 4.2f} [pC]'
                ax[1].text(1.125, 0.85, text, fontsize=12, va="top", ha="right", transform=ax[1].transAxes)
                # ax[1].set_xlim([0, 31.5])
                
                ax[2].plot(self.hstime, conversion_factors["TOF H"] * np.array(packets[(k[0], "TOF H")]))
                ax[2].set_ylabel("TOF H", font="Times New Roman", fontsize=15, fontweight='bold')
                text = f'Min = {min(conversion_factors["TOF H"] * np.array(packets[(k[0], "TOF H")])): 4.2f} [pC]'+ '\n'+ f'Avg={np.mean(conversion_factors["TOF H"] * np.array(packets[(k[0], "TOF H")])): 4.2f} [pC]'+ '\n' + f'Std={np.std(conversion_factors["TOF H"] * np.array(packets[(k[0], "TOF H")])): 4.2f} [pC]'+ '\n' + f'Max = {max(conversion_factors["TOF H"] * np.array(packets[(k[0], "TOF H")])): 4.2f} [pC]'
                ...
                text = f'Min = {min(conversion_factors["Ion Grid"] * np.array(packets[(k[0], "Ion Grid")])): 4.2f} [pC]'+ '\n'+ f'Avg={np.mean(conversion_factors["Ion Grid"] * np.array(packets[(k[0], "Ion Grid")])): 4.2f} [pC]'+ '\n' + f'Std={np.std(conversion_factors["Ion Grid"] * np.array(packets[(k[0], "Ion Grid")])): 4.2f} [pC]'+ '\n' + f'Max = {max(conversion_factors["Ion Grid"] * np.array(packets[(k[0], "Ion Grid")])): 4.2f} [pC]'
                ...
                if(self.header[(k[0], 'Timestamp')] < 494_733_600):  # If we are before September 27th, 2023 then we use the old definitions
                    ...
                    ax[4].plot(self.lstime, conversion_factors["Target L"] * np.array(packets[(k[0], "Target L")]))
                    ax[4].set_ylabel("Target LG", font="Times New Roman", fontsize=15, fontweight='bold')
                    text = f'Min = {min(conversion_factors["Target L"] * np.array(packets[(k[0], "Target L")])): 4.2f} [pC]'+ '\n'+ f'Avg={np.mean(conversion_factors["Target L"] * np.array(packets[(k[0], "Target L")])): 4.2f} [pC]'+ '\n' + f'Std={np.std(conversion_factors["Target L"] * np.array(packets[(k[0], "Target L")])): 4.2f} [pC]'+ '\n' + f'Max = {max(conversion_factors["Target L"] * np.array(packets[(k[0], "Target L")])): 4.2f} [pC]'
                    ax[4].text(1.125, 0.85, text, fontsize=12, va="top", ha="right", transform=ax[4].transAxes)
                    # ax[4].set_xlim([0, 126.5])
                    
                    ax[5].plot(self.lstime, conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")]))
                    ax[5].set_ylabel("Target HG", font="Times New Roman", fontsize=15, fontweight='bold')
                    text = f'Min = {min(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'+ '\n'+ f'Avg={np.mean(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'+ '\n' + f'Std={np.std(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'+ '\n' + f'Max = {max(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'
                    ax[5].text(1.125, 0.85, text, fontsize=12, va="top", ha="right", transform=ax[5].transAxes)
                    # ax[5].set_xlim([0, 126.5])

                else:
                    ax[4].plot(self.lstime, conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")]))
                    ax[4].set_ylabel("Target HG", font="Times New Roman", fontsize=15, fontweight='bold')
                    text = f'Min = {min(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'+ '\n'+ f'Avg={np.mean(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'+ '\n' + f'Std={np.std(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'+ '\n' + f'Max = {max(conversion_factors["Target H"] * np.array(packets[(k[0], "Target H")])): 4.2f} [pC]'
                    ax[4].text(1.125, 0.85, text, fontsize=12, va="top", ha="right", transform=ax[4].transAxes)
                    # ax[4].set_xlim([0, 126.5])
                    
                    ax[5].plot(self.lstime, conversion_factors["Target L"] * np.array(packets[(k[0], "Target L")]))
