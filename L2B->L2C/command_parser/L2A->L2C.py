...
output_path = "IDEX_L2C_Example.h5"
...
with h5py.File(output_path, "w") as h5file:
    for woy in sorted(df["WOY"].unique()):
        ...
        for _, row in df_woy.iterrows():
            ...
            # Compute total counts and distribute across lat/lon bins
            total_count = sum(row[col] for col in count_cols)
            lat_bins = 30
            lon_bins = 60
            num_bins = lat_bins * lon_bins

            if total_count > 0:
                # Randomly distribute total_count over all bins
                weights = np.random.dirichlet(np.ones(num_bins))
                latlon_counts = np.random.multinomial(total_count, weights)
            else:
                latlon_counts = np.zeros(num_bins, dtype=int)

            # Store as a 2D array: [lat_bin, lon_bin]
            latlon_array = latlon_counts.reshape((lat_bins, lon_bins))
            doy_group.create_dataset("latlon_bin_counts", data=latlon_array)
