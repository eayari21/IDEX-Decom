...
def FitTargetSignal(time, targetAmp):
    ...
    from scipy.optimize import curve_fit
    import pandas as pd
    # Assumes that detrend, LinearFit, SineFit, butter_lowpass_filter, and IDEXIonGrid are defined

    x = np.asarray(time)
    y = np.asarray(targetAmp)

    # || Select only raw noise (where we know the signal is not)
    mask = np.logical_and(time >= -55, time <= -50)
    print(mask)
    ...
    try:
        ...
        # Optionally subtract linearbase if desired:
        # y -= linearbase
        y = detrend(y)
    except Exception as e:
        print(f"Linear background not found due to {e}.")
        linearbase = None

    # || Remove Sine Wave Background
    try:
        # Note: 'basedex' should be defined elsewhere in your code.
        baselinedelined = y[mask]
        sineparam, sine_cov = curve_fit(SineFit, baselinedomain, baselinedelined, 
                                        p0=[max(baselinedelined), 7000, 45], maxfev=100_000)
        sinebase = SineFit(time, sineparam[0], sineparam[1], sineparam[2])
        y -= sinebase
        # y = butter_lowpass_filter(y, time)
    except Exception as e:
        print(f"Sinusoidal background not found: {e}.")
        sinebase = None

    # || Restrict to signal region
    mask = np.logical_and(time >= -30, time <= 30)
    # x = time[mask]
    # y = y[mask]

    pre = -30.0  # Before image charge

    # Set elements of y to nan where x >= pre to define the baseline region
    yBaseline = np.where(x <= pre, y, np.nan)
    yImage = y[(x >= pre) & (x < 0.0)]
    
    ionError = np.std(yBaseline)  # Uncertainty estimated from the baseline region
    ionMean = np.nanmean(yBaseline)
    yBaseline -= ionMean

    # Convert to floats for the fitting process
    ionTime = np.array([float(val) for val in x])
    ionAmp  = np.array([float(val) for val in y])
    ...
    A  = max(ionAmp)  # amplitude (v)
    t1 = 0.371    # rise time (s)
    t2 = 0.371    # discharge time (s)

    # Fit the ion grid signal model to the data
    param, param_cov = curve_fit(IDEXIonGrid, ionTime, ionAmp, p0=[t0, c, A, t1, t2], maxfev=100_000)

    # Evaluate the fitted model
    model = IDEXIonGrid(ionTime, param[0], param[1], param[2], param[3], param[4])
    print(f"***Model = {model}***")
    sig_amp = max(model) - np.mean(model[:80])
    print(f"***sig_amp = {sig_amp}***")


    # Compute residuals between the data and model
    residuals = ionAmp - model

    # Calculate chi-squared using ionError as the uncertainty if available.
    # If ionError is 0 or not valid, it falls back to the sum of squared residuals.
    if ionError > 0:
        chi2 = np.sum((residuals / ionError) ** 2)
    else:
        chi2 = np.sum(residuals ** 2)

    # Degrees of freedom: number of data points minus the number of parameters
    ndof = len(ionAmp) - len(param)
    red_chi2 = chi2 / ndof if ndof > 0 else np.nan

    return (param, param_cov, sig_amp, chi2, red_chi2)
            ...
            if 'IDX__SCI0TYPE' in pkt.data:
                ...
                if pkt.data['IDX__SCI0TYPE'].raw_value == 1:
                    ...
                    if(pkt.data['IDX__TXHDRLGTRIGMODE'].derived_value!=0):
                        ...
                        self.header[(evtnum, 'TriggerLevel')] = 5.14e-4*((pkt.data['IDX__TXHDRLGTRIGCTRL1'].derived_value >> 22) & mask_10_bit)
                    ...
                    if(pkt.data['IDX__TXHDRMGTRIGMODE'].derived_value!=0):
                        ...
                        # Extract the last 10 bits (bits 0-9)
                        self.header[(evtnum, 'TriggerLevel')] = 1.13e-2*((pkt.data['IDX__TXHDRMGTRIGCTRL1'].derived_value >> 22) & mask_10_bit)
                        print(f"Trigger level = {self.header[(evtnum, 'TriggerLevel')]}")
    ...
    def write_to_hdf5(self, waveforms: dict, filename: str):
        ...
        #     create_dataset_if_not_exists(h, f"/{k[0]}/SpiceData/Ephemeris/VelocityZ", velocity_z)
            ...
            if(f"/{k[0]}/Metadata/Epoch" not in h):
                day_of_year = datetime.fromtimestamp(self.header[(int(k[0]), 'Timestamp')]).timetuple().tm_yday
                create_dataset_if_not_exists(h, f"/{k[0]}/Time [DOY]", data = day_of_year)

                spin_phase = np.random.choice([0, 90, 180, 270])
                create_dataset_if_not_exists(h, f"/{k[0]}/SpinPhase [degrees]", data = spin_phase)
            ...
            if k[1] in conversion_factors:
                ...
                if k[1] in ['Target L', 'Target H', 'Ion Grid']:  # Fit target and ion grid signals
                    try:
                        print(f"||====|| {k[1]} = {v} ||====||")
                        print(f"||====|| t = {self.lstime} ||====||")

                        param, param_cov, sig_amp, chi2, rchi2 = FitTargetSignal(self.lstime, transformed_data)
                        create_dataset_if_not_exists(h, f"/{k[0]}/Analysis/{k[1]}FitParams", data=np.array(param))
                        create_dataset_if_not_exists(h, f"/{k[0]}/Analysis/{k[1]}ChiSquared", data=chi2)
                        create_dataset_if_not_exists(h, f"/{k[0]}/Analysis/{k[1]}ReducedChiSquared", data=rchi2)

                        create_dataset_if_not_exists(h, f"/{k[0]}/{k[1]}Mass", data=sig_amp)
                        create_dataset_if_not_exists(h, f"/{k[0]}/{k[1]}ImpactCharge", data=sig_amp)
                    except Exception as e:
                        print(f"Failed to fit impact charge waveforms due to {e}.")
